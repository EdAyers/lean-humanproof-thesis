...
| c e :=
    let r := (@except_t.run (option expr) id _ (expr.coord.modify (λ Γ e, throw e) c e)) in
...
namespace zipper
    ...
    meta def mfold_up {T} [monad T] {α} (f : α → zipper → T α) : α → zipper → T α
    ...
    meta def altfold {T} [monad T] [alternative T] {α} (f : α → zipper → T α) : α → zipper → T α
    | a z := (do a ← f a z, z.children.mfoldl altfold a) <|> pure a

    meta def altfold_up {T} [monad T] [alternative T] {α} (f : α → zipper → T α) : α → zipper → T α
    | a z := (f a z >>= λ a, option.rec_on (up z) (pure a) (altfold_up a)) <|> pure a

    meta def pick_up {T} [monad T] [alternative T] {α} (f : zipper → T α) : zipper → T α
    ...
    /-- True when the cursor is _in_ the arg of an app. That is, the path directly above is `app_arg`. -/
    meta def is_app_arg : zipper → bool
    |⟨(entry.app_arg _ _)::t,_⟩ := tt |_ := ff

    /-- Make fresh local_constants for each item in the context and instantiate the cursor with them. -/
    ...
    -- meta def find_vars : ℕ → zipper → list zipper | n z :=
    --     match z.current with
    --     |(expr.var i) := if i = n then [z] else []
    --     |_ := if ¬ z.current.has_var_idx n then [] else z.children_with_depths.collect (λ ⟨z,d⟩, find_vars (n+d) z)
    --     end
    ...
    | z n := match z.cursor with
      ...
      | c :=
        ...
        z.children.collect (λ z, find_var z n)
      end
