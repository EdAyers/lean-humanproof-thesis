...
section
  /-- Implementation of writer monad with append (instead of multiply as used by mathlib's writer_t). -/
...
namespace alternative
  ...
  /-- Generalisation of `tactic.returnopt`. -/
  ...
  /-- Repeatedly perform `t : T α` until it fails, collect the resulting `α` in a list and return. -/
...
/-- monad_except lifts through state_t.  -/
...
, catch := λ α S e, ⟨λ s, catch (state_t.run S s) (λ x, state_t.run (e x) s)⟩
}

/-- monad_except ⇒ alternative -/
...
/-- Perform the given stateful `m α`, throw away the state and return just the value. -/
